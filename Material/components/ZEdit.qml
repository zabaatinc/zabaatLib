...
Item
{
    ...
    property bool resizeVisible  	  : true
    ...
    //GENERATES A string that can be saved as a QML
    ...
    Rectangle {
        ...
        MouseArea   {
            ...
            onDragActiveChanged: {
                if(!dragActive) {
                    zReleased(theParent.objectName + ".qml",theParent.x,theParent.y)
					if(restorePosOnDrop)
						restorePos()
				}
                else
    ...
    Rectangle {
        ...
        visible :theParent && resizeVisible ? true : false
        ...
        onPosChanged : if(rotationArea.dragActive) calcAngle(pos.x, pos.y)
//        Connections {
//            target : theParent
//            onRotationChanged : console.log(theParent.rotation)
//        }
...
//            rootObject.oldRot = rootObject.rotation
    ...
    Rectangle {
        ...
        radius : 7
        visible : enabled
        enabled : theParent && resizeVisible ? true : false
    ...
    function getPropArr() {
       return theParent ? theParent.propArr : null;
    }
    function restorePos() {
        if(theParent) {
            theParent.x = origX
