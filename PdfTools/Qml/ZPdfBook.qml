...
TabView
{
    ...
    //TODO , this is assuming the pdfWriter (C++) succeeded in saving. Need to implement a signal from the C++ side that sends a signal out and then
    //connect to it instead!
    signal saved(string pdfLocation)
    ...
    property int cmpStatus :
    {
        for(var i = 0; i < tv.count; i++)
        {
            var section = getTabItem(i)
            if(section.status != Component.Ready)
    ...
    onPdfWriterPtrChanged:
    {
        if(pdfWriterPtr && typeof pdfWriterPtr.finishedSaving !== 'undefined')
            pdfWriterPtr.finishedSaving.connect(saved)

    }


    function bodyWidthPx() { return ((pgWidthInches - lMargin - rMargin)/pgWidthInches) * tv.width }
    ...
    function setEditMode(value, editOptions)
    {
        ...
        {
            if(getTabItem(i))
                getTabItem(i).setEditMode(value, editOptions)
        }
    }
    ...
    property var grabImages : function(directory)
    {
        if(pdfWriterPtr)
        {
            if(directory)
    ...
    function saveStr(tabStr)
    {
        ...
        str += tabStr + '\tproperty int status            : dataSectionStatus == Component.Ready && cmpStatus == Component.Ready ? Component.Ready : Component.Loading' + "\n"
        str += tabStr + '\tproperty int dataSectionStatus : Component.Loading'    + "\n"

        if(pdfWriterPtr)
        {
            var pdfInfo = pdfWriterPtr.updatePdfInfoInDataSection()
            var pdfStr = "{"
            for(var p in pdfInfo)
                pdfStr += "\n\t\t\t\t\t\t\t\t\t\t" + tabStr +  p + ":" + pdfInfo[p] + ","
            pdfStr = pdfStr.substring(0, pdfStr.length-1)
            pdfStr += "}"
        }

        str += tabStr + '\tproperty var dataSection :  ({ pdf : ' + pdfStr +  " })\n"
        str += tabStr + '\tfunction setDataSection(obj)\n' +
               tabStr + '\t{\n' +
               tabStr + '\t\tfor(var o in obj)\n' +
               tabStr + '\t\t\tdataSection[o] = obj[o]\n' +
                tabStr + '\t\t\trootObject.dataSectionStatus = Component.Ready\n' +
               tabStr + '\t}\n'
...
//                childstr = childstr.substr(0,childstr.length - 2) + tabStr + "\t\t\t sectionName:parent.title\n" + tabStr + "\t\t}"
                childstr = childstr.split('dataSection').join('rootObject.dataSectionStatus == Component.Ready ? rootObject.dataSection').split('\n')

                for(var j in childstr)
                {
                    if(childstr[j].indexOf('dataSection') != -1)
                        childstr[j] += ": 0"
                }

                childstr = childstr.join('\n')
