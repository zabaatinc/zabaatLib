...
using namespace std;
class wolfsubmodel : public QQmlListModel {
    Q_OBJECT
...
protected:
    QRoles roleNames() const {
        ...
        QRoles r;
        try {
            wolfsubmodel &s = dynamic_cast<wolfsubmodel&>(*source);
            r = s.roleNames();
        }
...
public:
    wolfsubmodel(QObject * parent = 0) : QQmlListModel(parent) {
        m_count = 0;
        m_readOnly = true;
        source = NULL;
...
private:
    ...
    void connectSignals(QQmlListModel *src) {
        conn_rowsInserted = connect(src, &QQmlListModel::rowsInserted, this, &wolfsubmodel::__rowsInserted);
        conn_rowsMoved    = connect(src, &QQmlListModel::rowsMoved   , this, &wolfsubmodel::__rowsMoved   );
        conn_rowsRemoved  = connect(src, &QQmlListModel::rowsRemoved , this, &wolfsubmodel::__rowsRemoved );
        conn_dataChanged  = connect(src, &QQmlListModel::dataChanged , this, &wolfsubmodel::__dataChanged );
        conn_modelReset   = connect(src, &QQmlListModel::modelReset  , this, &wolfsubmodel::__modelReset  );
    }
