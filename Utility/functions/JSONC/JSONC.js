...
//  toString = {}.toString;
  ...
  function _compressArray(json, aKeys) {
    ...
    for (nIndex = 0, nLenKeys = json.length; nIndex < nLenKeys; nIndex++) {
      json[nIndex] = compress(json[nIndex], aKeys);
    }
  }
...
function pack (json, bCompress) {
  var str = JSON.stringify((bCompress ? compress(json) : json));
  return Base64.Base64.encode(String.fromCharCode.apply(String, Gzip.zip(str,{level:9})));
};
  /**
  ...
  function unpack(gzipped, bDecompress) {
    var aArr = getArr(Base64.Base64.decode(gzipped)),
