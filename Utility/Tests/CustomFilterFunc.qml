...
Item {
    ...
    Row {
        ...
        ZTextBox {
            ...
            onTextChanged: logic.makeNewFilter(textFirst.text,textLast.text)
        }
        ZTextBox {
            id : textLast
            label : "lastName"
    ...
    QtObject {
        ...
        function makeNewFilter(f,l){
            subModel.filterFunction = function(a) {
                return a.first.toLowerCase() == f.toLowerCase() || a.last.toLowerCase() == l.toLowerCase()
            }

            subModel2.queryTerm = ({"$or":[
                                           {first:{"==":f}},
                                           {last :{"==":l}}
                                          ]
                                   })
