//This one returns string to the QJS runtime instead of objects. This does mean that we
//have additional overhead:
//c++ ---> string --> object ---> string ---> js ---> object
//instead of
//c++ --> string ---> object ---> js
//The reason for leaving this here is because passing strings might be much safer than passing objects
...
//                                                       qDebug () << "C++ EVENT FIRED::" << QString::fromStdString(ev.get_name());
                                                       QJsonArray arr;
                                                       addMsgToJsonArray(ev.get_message(), 0, arr);
                                                       if(arr.size() > 0) {
                                                            QJsonDocument doc(arr);
                                                            Q_EMIT serverResponse(QString::fromStdString(ev.get_name()), QString(doc.toJson()) , "");
                                                       }
...
Q_SIGNALS:
    ...
    void failed();
    void serverResponse(QString eventName, QString value, QString cbId);
...
//                            args.push_back(v);
//                        }
                        qDebug() << rtt.stop() << " ms taken to get callback" ;
...
//        QVariantList var;
        ...
        for(uint i = 0; i < list.size(); i++){
             ...
             addMsgToJsonArray(list[i] , 0, arr);
        }
        qDebug() << convTimer.stop() << "ms taken for " << eventName;

        Q_EMIT serverResponse(eventName,
                              QString(QJsonDocument(arr).toJson()),
                              cbId);
    }
    void addMsgToJsonArray(const sio::message::ptr &msg, uint depth, QJsonArray &rArr){
        auto flag = msg->get_flag();
        ...
        switch(flag){
            ...
            case sio::message::flag_binary : {  //this is for callbacks!
                std::shared_ptr<const std::string> bin = msg->get_binary();
                rArr.push_back(QString::fromStdString(*bin));
                return;
            }
            case sio::message::flag_boolean : {
                rArr.push_back(msg->get_bool() );
                return ;
            }
            case sio::message::flag_double : {
                rArr.push_back(msg->get_double());
                return;
            }
            case sio::message::flag_integer : {
                rArr.push_back((int)msg->get_int());
                return;
            }
            case sio::message::flag_string : {
                rArr.push_back(QString::fromStdString(msg->get_string()));
                return;
            }
            case sio::message::flag_null : {
                rArr.push_back(QJsonValue::Null);
                return;
            }
            case sio::message::flag_object: {
                auto &map = msg->get_map();
