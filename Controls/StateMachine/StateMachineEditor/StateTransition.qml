...
//    onRulesChanged: console.log(JSON.stringify(rules,null,2))
    ...
    function getJSON(){
        return { name : name, rules : rules, state: destinationName }
    }
    ...
    QtObject {
        ...
        function runUpdate(){
            ...
            var total     = origin.transitions.children.length
            var multi     = total === 1 ? destIndex === -1 ? 0.5 : 0
...
//                console.log("4")
        ...
        function getIndex(target, prop){
            if(target && target.transitions) {
                for(var i = 0; i < target.transitions.children.length; i++){
                    var item = target.transitions.children[i]
